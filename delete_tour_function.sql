...
CREATE OR REPLACE FUNCTION delete_tour_safely(tour_id UUID, user_id UUID)
RETURNS BOOLEAN
...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1 FROM tours WHERE id = tour_id AND creator_id = user_id
  ) INTO is_creator;
  ...
  IF is_creator OR is_admin THEN
    ...
    -- Delete ratings
    DELETE FROM ratings WHERE tour_id = tour_id;
    
    -- Delete notifications
    DELETE FROM notifications WHERE tour_id = tour_id;
    
    -- Delete bookings for all schedules of this tour
    DELETE FROM bookings 
    WHERE schedule_id IN (SELECT id FROM schedules WHERE tour_id = tour_id);
    
    -- Delete schedules
    DELETE FROM schedules WHERE tour_id = tour_id;
    
    -- Finally delete the tour itself
    DELETE FROM tours WHERE id = tour_id;
