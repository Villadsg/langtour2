		...
		<div class="flex justify-between h-18">
			<div class="flex">
				<div class="flex-shrink-0 flex items-center">
					<a href="/" class={`text-xl font-semibold tracking-tight ${text.primary}`}>ClassRoute</a>
				</div>
				<div class="hidden sm:ml-8 sm:flex sm:space-x-8">
					<a
						href="/"
						class={`inline-flex items-center px-1 pt-1 text-sm font-medium ${isActive('/') ? navbar.activeLink : navbar.link}`}
					>
						Book a route
					</a>
					<a
						href="/create-with-ai"
						class={`inline-flex items-center px-1 pt-1 text-sm font-medium ${isActive('/create-with-ai') ? navbar.activeLink : navbar.link}`}
					>
						Create a route
					</a>
				</div>
			...
			<div class="hidden sm:ml-6 sm:flex sm:items-center">
				...
				{:else}
					<div class="flex items-center space-x-4">
						<a href="/login" class={components.button.secondary}>Login</a>
						<a href="/signup" class={components.button.primary}>Sign Up</a>
					</div>
				{/if}
			</div>
	...
	{#if isMenuOpen}
		<div class={`sm:hidden ${navbar.mobileBg}`}>
			<div class="py-2 space-y-1">
				<a href="/" onclick={closeMenu} class="block px-4 py-2 text-base font-medium">
					Book a route
				</a>
				<a href="/create-with-ai" onclick={closeMenu} class="block px-4 py-2 text-base font-medium">
					Create a route
				</a>
				...
				{:else}
					<div class="space-y-1">
						<a href="/login" onclick={closeMenu} class="block px-4 py-2 text-base font-medium">Login</a>
						<a href="/signup" onclick={closeMenu} class="block px-4 py-2 text-base font-medium">Sign Up</a>
					</div>
				{/if}
			</div>
		</div>
