	...
	<p class="text-slate-900 leading-loose">
		{#each phrase.words as w, i (i)}<span class="relative inline-block"
				><button
					...
					>{w.word}</button
				>{#if w.meaning}<span
						class="pointer-events-none absolute bottom-full left-1/2 z-10 mb-1 hidden -translate-x-1/2 whitespace-nowrap rounded-md border border-slate-200 bg-white px-2 py-0.5 text-sm text-emerald-700 shadow-sm peer-hover:block peer-focus:block"
						>{w.meaning}</span
					>{/if}</span
	...
	<p class="text-slate-900 leading-loose">
		<span class="relative inline-block"
			...
			>{#if phrase.sentenceTranslation}<span
					class="pointer-events-none absolute bottom-full left-1/2 z-10 mb-1 hidden w-max max-w-[90vw] -translate-x-1/2 whitespace-normal rounded-md border border-slate-200 bg-white px-2 py-1 text-sm leading-snug text-emerald-700 shadow-sm peer-hover:block peer-focus:block"
