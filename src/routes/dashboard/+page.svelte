...
<div class="container mx-auto px-4 py-8">
  <h1 class="text-3xl font-medium text-slate-700 mb-6">Your Dashboard</h1>
  ...
  {:else}
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-medium text-slate-700">Created Trails</h2>
      <div class="flex gap-3">
        <a href="/dashboard/create" class="bg-slate-600 hover:bg-slate-700 text-white font-medium py-2.5 px-5 rounded-lg transition-colors">
          Create New Trail
        </a>
        <a href="/create-with-ai" class="bg-slate-100 hover:bg-slate-200 text-slate-600 font-medium py-2.5 px-5 rounded-lg transition-colors">
          Create with AI Chat
        </a>
      </div>
    </div>
    ...
    {#if deleteSuccess}
      <div class="relative flex items-center justify-between bg-slate-50 border border-slate-200 text-slate-700 px-4 py-3 rounded-lg mb-4" role="alert">
        <p>{deleteSuccess}</p>
        <button class="ml-4 text-slate-400 hover:text-slate-600" on:click={() => deleteSuccess = ''}>
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
        </button>
      </div>
    {/if}
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {#each $userCreatedTours as tour}
          ...
          <div class="bg-white rounded-lg border border-slate-200 overflow-hidden hover:shadow-sm transition-shadow">
            ...
            {/if}
            <div class="p-4">
              <h3 class="text-lg font-medium text-slate-700 mb-2">{tourData.name}</h3>
              <p class="text-slate-600 mb-4 line-clamp-2">{tourData.description}</p>
              <div class="flex space-x-4 justify-end mb-2">
                <a href={`/dashboard/edit/${tour.id || tour.$id}`} class="text-slate-500 hover:text-slate-600 font-normal transition-colors">
                  Edit Trail
                </a>
                <a href={`/dashboard/tours/${tour.id || tour.$id}/schedule`} class="text-slate-500 hover:text-slate-600 font-normal transition-colors">
                  Schedule Trail
                </a>
                <button
                  class="text-red-400 hover:text-red-500 cursor-pointer font-normal transition-colors {deletingTourId === (tour.id || tour.$id) ? 'opacity-50 cursor-not-allowed' : ''}"
    ...
    <div class="mt-8">
      <h3 class="text-lg font-medium text-slate-700 mb-4">All created schedules</h3>
      ...
      {#if successMessage}
        <div class="relative flex items-center justify-between bg-slate-50 border border-slate-200 text-slate-700 px-4 py-3 rounded-lg mb-4" role="alert">
          <p>{successMessage}</p>
          <button class="ml-4 text-slate-400 hover:text-slate-600" on:click={() => successMessage = ''}>
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
        </div>
      {/if}
                ...
                <tr class="hover:bg-slate-50">
                  <td class="py-3 px-4">
                    {#if schedule.tours}
                      ...
                      <span class="text-slate-600">{tourData.name}</span>
                    {:else}
                  ...
                  <td class="py-3 px-4">
                    <a
                      ...
                      class="text-slate-500 hover:text-slate-600 font-normal transition-colors mr-3"
                    >
                      Manage
                    </a>
                    <button
                      class="text-red-400 hover:text-red-500 font-normal transition-colors {cancellingScheduleId === schedule.id ? 'opacity-50 cursor-not-allowed' : ''}"
