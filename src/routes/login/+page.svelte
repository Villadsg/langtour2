...
<div class="min-h-screen bg-slate-50 flex flex-col pt-24 sm:px-6 lg:px-8">
  <div class="sm:mx-auto sm:w-full sm:max-w-md">
    ...
    <p class={`mt-2 text-center text-sm ${text.secondary}`}>
      Or
      <a href="/signup" class="font-medium text-slate-600 hover:text-slate-500">
    ...
    <div class="bg-white py-6 px-4 shadow sm:rounded-lg sm:px-8">
      {#if emailConfirmationNeeded}
        <div class="mb-4 bg-slate-50 border border-slate-200 text-slate-600 px-4 py-3 rounded-md">
          <p class="font-normal">{generalError}</p>
          <p class="mt-2 text-sm">Didn't receive the confirmation email? Check your spam folder or <button class="text-slate-600 underline font-normal" on:click={() => ConvexService.resendConfirmationEmail(email)}>resend it</button>.</p>
        </div>
        ...
        <div>
          ...
          <div class="mt-1">
            <input
              ...
              autocomplete="email"
              bind:value={email}
              class="appearance-none block w-full px-3 py-2 border border-slate-200 rounded-lg shadow-sm placeholder-slate-400 focus:outline-none focus:ring-slate-300 focus:border-slate-300 sm:text-sm"
            />
            {#if emailError}
